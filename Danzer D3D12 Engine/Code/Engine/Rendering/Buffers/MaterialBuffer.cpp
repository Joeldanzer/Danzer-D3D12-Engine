...
void MaterialBuffer::Init(ID3D12Device* device, DescriptorHeapWrapper* cbvWrapper)
{
	HRESULT result;
	...
	for (UINT i = 0; i < FrameCount; i++)
	{
		...
		UINT size = (sizeof(MaterialBuffer::Data) + 255) & ~255;
		...
		CHECK_HR(result);
		memcpy(m_bufferGPUAddress[i], &m_materialBufferData, sizeof(MaterialBuffer::Data));
	}
}

void MaterialBuffer::UpdateBuffer(/*ID3D12GraphicsCommandList* cmdList,*/ UINT frameIndex, void* cbvData)
{
	Data* data = reinterpret_cast<Data*>(cbvData);
	m_materialBufferData = *data;
	memcpy(m_bufferGPUAddress[frameIndex], &m_materialBufferData, sizeof(Data));
}
